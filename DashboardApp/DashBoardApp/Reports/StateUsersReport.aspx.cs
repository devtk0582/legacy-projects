...
{
    public partial class StateUsersReport : System.Web.UI.Page
    {
        ...
        private void BindGrid(int stateId)
        {
            try
            {
                ...
                var stateUsers = (new BOUsers()).GetStateUsers(stateId);
                if (stateUsers != null && stateUsers.Count > 0)
                {
                    dlReport.DataSource = stateUsers;
                    dlReport.DataBind();
                    lblDataMsg.Text = "";
                }
                else
